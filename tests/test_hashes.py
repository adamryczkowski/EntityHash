...
def testPydanticModelHash():
    model = TestModel(name="test", value=42)
    entity_hash = EntityHash.FromPydanticModel(model)
    assert isinstance(entity_hash, EntityHash)
    assert entity_hash.as_base64 == "mjBL6CkTTb5rbMxU5+eObgtHfEinW44wQjidvWFhdWk="
    assert entity_hash.as_hex is not None
    assert entity_hash.as_int is not None
...
def test_file():
    ...
    entity_hash1 = EntityHash.HashDiskFile(Path(tmp_file.name), "sha256")
